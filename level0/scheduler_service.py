...
class SchedulerService:
    ...
    def schedule_once(
        ...
        if run_time < now - timedelta(seconds=_PAST_TOLERANCE_SEC):
            raise ValueError(
        ...
        logger.info("Scheduled once: %s at %s", scheduled_id, run_time.isoformat())
        return scheduled_id
    ...
    def _execute_once(self, scheduled_id: str):
        ...
        try:
            ...
            self.collection.update_one(
                ...
                {"$set": {
                    "status":                "completed",
                    "last_execution_job_id": execution_job_id,
                }},
            )
        except Exception as exc:
            logger.error("Scheduled once job failed: %s — %s", scheduled_id, exc)
            self.collection.update_one(
                {"scheduled_id": scheduled_id},
    ...
    def _execute_interval(self, scheduled_id: str):
        ...
        try:
            ...
            logger.error("Interval job failed: %s — %s", scheduled_id, exc)
            error_msg = str(exc)
        finally:
